...
def parse_args():
    global H, W, NUM_BLOCKING, BRD_SIZE, BLOCKING_NBRS, EDGES
    ...
    BLOCKING_NBRS = [[] for _ in range(BRD_SIZE)]
    for i in range(BRD_SIZE):
        for drt in [W, -W, 1, -1]:  # south, north, east, west
            ...
            if nbrs: BLOCKING_NBRS[i].append(nbrs)
...
def disconnected(brd, pos=0):  # returns set of non-blocking positions that are disconnected from the rest (floodfill)
    seen = set()
    queue = [brd.find('-') if not pos else pos]
...
def block_choices(brd):  # sort choices for '#' placement to avoid "clumps" and therefore minimize word length
    ...
    for pos in choices:
        ...
        if not new_brd:
            choices[pos] = False
            continue
        choices[pos] = clump_score(new_brd)
...
def clump_score(brd):
    ...
    for i, char in enumerate(brd):
        ...
        for idx in queue:  # loop through queue of blocking squares and their neighbors
            if i in EDGES: score -= 0.2  # prioritize having '#' in the middle, not the edges
            if not 0 <= idx < BRD_SIZE or idx in seen or brd[idx] != '#': continue  # base case for floodfill
            seen.add(idx)
            if (idx + 1) // W == idx // W: queue.append(idx + 1)  # check each neighbor of every '#' without wrap around
            if (idx - 1) // W == idx // W: queue.append(idx - 1)
            queue.append(idx + W)
            queue.append(idx - W)
    return score
...
def setup(brd):  # pre-compute possible words at each position
