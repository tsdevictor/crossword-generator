...
def parse_args():
    ...
    WORDS_BY_LEN = {i: sorted(WORDS_BY_LEN[i], key=(lambda w: -sum(w.count(c) * value[c] for c in alphabet))) for i in
                    WORDS_BY_LEN}
    ...
    if NUM_BLOCKING % 2: brd[
        BRD_SIZE // 2] = '#'  # if odd num_blocking, '#' must be in center because board is symmetric over 180
...
def is_valid(brd):  # check that each word (i) exists or (ii) is not yet complete
    ...
    for start, orientation in POSSIBLE_WORDS:
        ...
        for i in POSSIBLE_POSITIONS[(start, orientation)]:
            if brd[i] == '-':
                word = word[:min(i, 3)]
                if len(word) >= 2 and word not in PREFIXES: return ''
                break
            word += brd[i]
        else:
