...
args = ['dctEckel.txt', '15x15', '37', 'H0x4#', 'v4x0#', 'h6x5a']


def parse_args():
    global WORDS_BY_LEN, H, W, NUM_BLOCKING, BRD_SIZE, ALL_WORDS, WORDS_BY_LEN, BLOCK_NBRS
...
def update_words(brd, positions, orientation, possible_positions, possible_words, used_words):
    ...
    for pos in positions:
        ...
        updated_words[(start, opposite)] = [w for w in possible_words[(start, opposite)]
                                            if w[(pos - start) // opposite] == brd[pos] and w not in used_words]
...
def fill_words(brd, possible_positions, possible_words, used_words):  # brute force filling of words in xword
    ...
    for word in possible_words[(start, orientation)]:
        ...
        upd_p, upd_w = update_words(new_brd, positions, orientation, possible_positions, possible_words, used_words | {word})
        new_brd = fill_words(new_brd, upd_p, upd_w, used_words | {word})
        if new_brd: return new_brd
    return ''
