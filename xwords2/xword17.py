...
import re
args = ['dct20k.txt', '6x6', '0']


def parse_args():
    global WORDS_BY_LEN, H, W, NUM_BLOCKING, BRD_SIZE, ALL_WORDS, WORDS_BY_LEN, BLOCK_NBRS, EDGES
    ...
    for i in range(BRD_SIZE):
        if i % W == 0 or i % W == W - 1: EDGES.add(i)
        if i // W == 0 or i // W == H - 1: EDGES.add(i)

    ALL_WORDS = set()  # all prefixes, all words
...
def place_block(brd, pos, item):
    ...
    dc = disconnected(''.join(brd))  # if any white squares are not connected, fill them...
...
def block_choices(brd):
    choices = {i: 0 for i in range(BRD_SIZE) if brd[i] == '-'}
    for pos in choices:
        new_brd = place_block(brd, pos, '#')
        if not new_brd:
            choices[pos] = False
            continue
        choices[pos] = clump_score(new_brd)  # if any white squares are not connected, fill them...
    choices = {i: choices[i] for i in choices if choices[i]}
    return sorted(choices.keys(), key=lambda ch: choices[ch], reverse=True)


def clump_score(brd):  # returns set of non-blocking positions that are disconnected from the rest (floodfill)
    seen = set()
    score = 0
    for i, char in enumerate(brd):
        if char != '#': continue
        queue = [brd.find('#', i)]
        if queue[0] in seen: continue
        score += 1
        for idx in queue:  # loop through queue of white squares and their neighbors
            if i in EDGES: score -= 0.2
            if not 0 <= idx < BRD_SIZE or idx in seen or brd[idx] != '#': continue
            seen.add(idx)
            if (idx + 1) // W == idx // W: queue.append(idx + 1)
            if (idx - 1) // W == idx // W: queue.append(idx - 1)
...
def block_structure(brd):  # brute force creation of blocking square structure
    ...
    for i in block_choices(brd):  # range(0, BRD_SIZE, 5):
        if brd[i] != '-': continue
...
def set_up(brd):  # pre-compute possible words at each position
    global POSSIBLE_WORDS, POSSIBLE_POSITIONS, POS_TO_WORD_START, MAX_SCORE
    MAX_SCORE = 0
    ...
    for key in POSSIBLE_POSITIONS:
        ...
        if word in ALL_WORDS and word not in seen:
            ...
            if score > MAX_SCORE:
                print(brd)
                MAX_SCORE = score
        else: unfilled.add(key)
    for key in unfilled:
        positions = POSSIBLE_POSITIONS[key]
        word = brd[positions[0]: positions[-1] + 1: key[1]]
        if '-' not in word: return ''
    return brd


def update_words(brd, positions, orientation, possible_positions, possible_words, used_word):
...
def fill_words(brd, possible_positions, possible_words):  # brute force filling of words in xword
    ...
    for word in possible_words[(start, orientation)]:
        ...
        upd_p, upd_w = update_words(new_brd, positions, orientation, possible_positions, possible_words, word)
