...
def parse_args():
    ...
    with open(args[0]) as dct:
        for line in dct:
            ...
            for char in line:
                if char.lower() not in 'abcdefghijklmnopqrstuvwxyz': break
            else:
                if len(line) not in WORDS_BY_LEN: WORDS_BY_LEN[len(line)] = []
                WORDS_BY_LEN[len(line)].append(line)
    ...
    for pos in range(BRD_SIZE):
        for drt in [W, -W, 1, -1]:
            nbrs, row_diff = [], abs(drt) // W
            for dist in range(1, 4):  # distance
                nbr = pos + dist * drt
                if not 0 <= nbr < W*H or abs(nbr // W - pos // W) != dist * row_diff: break
                nbrs.append(nbr)
...
def place(brd, pos, item):
    ...
    for i, char in enumerate(copy):
        ...
        for drt in NBRS_BY_DIRECTION[i]:
            for j in range(3):
                if not ('-#' in ''.join(copy[nbr] for nbr in drt) or ''.join(copy[nbr] for nbr in drt) in ('-', '--')): break
                if brd[drt[j]] != '-': return ''
...
def brute_force(brd):
    ...
    if count == NUM_BLOCKING: return brd.replace('.', '-')
