...
def get_disconnected(brd):  # returns (possibly empty) set of white squares that are disconnected from the rest (floodfill)
    if brd in CONNECTED_CACHE: return CONNECTED_CACHE[brd]
    seen, unseen = set(), {i for i in range(BRD_SIZE) if brd[i] != '#'}
    queue = [brd.find('-')]
    ...
    CONNECTED_CACHE[brd] = seen, unseen
    return seen, unseen  # return the two disconnected pieces
...
def place_block(brd, pos, item):  # place a character and all the implied characters that follow from it
    ...
    for i in queue:
        ...
        for drt in THREE_AWAY_NBRS[i]:  # drt means direction -> check word above, below, to the right, to the left
            # fill word adjacent to '#' with '#' characters if its len < 3 (see if statement below) (dst means distance)
            for dst in range(len(drt) if len(drt) < 3 else ''.join([brd[nbr] for nbr in drt]).find('#')):
                if brd[drt[dst]] != '#': queue.append(drt[dst])

    # there will only ever be at most two disconnected sections because any others would have been dealt with in a previous iteration
    for disconnected in get_disconnected(''.join(brd)):
        if len(disconnected) <= NUM_BLOCKING - brd.count('#'):
            if {brd[d] for d in disconnected} | {'-'} != {'-'}: continue
            for d in disconnected: brd[d], brd[reflect(d)] = '#', '#'   # loop through to fill up disconnected section
            return ''.join(brd)
    return ''


def block_choices(brd):  # sort choices of blocking square placement to avoid "clumps" and minimize word lengths
    choices = {i: False for i in range(BRD_SIZE) if brd[i] == '-'}
    for pos in choices:
...
CLUMP_SCORE_CACHE = {}  # cache blocking square configuration score
def clump_score(brd):
    if brd in CLUMP_SCORE_CACHE: return CLUMP_SCORE_CACHE[brd]
    score, seen = 0, set()
...
def word_setup(brd):  # pre-compute possible words at each position
    global ALL_WORDS, POSSIBLE_WORDS, POSSIBLE_POSITIONS, POS_TO_WORD_START
    ...
    char_value = {char: 0 for char in 'abcdefghijklmnopqrstuvwxyz'}  # compute "value" of each char based on how often it appears
    ...
    for line in open(args[0]):  # loop through dictionary
        if re.search(regex, (word := line.rstrip().lower())):
            ...
            ALL_WORDS.add(word)
            for char in word: char_value[char] += 1
    # sort words by their value (sum of values of each char in the word)
    words_by_len = {i: sorted(words_by_len[i], key=lambda w: sum(char_value[c] for c in w), reverse=True) for i in words_by_len}
    ...
    POS_TO_WORD_START = {(pos, orientation): i for (i, orientation) in POSSIBLE_POSITIONS for pos in POSSIBLE_POSITIONS[(i, orientation)]}
...
def place_word(brd, word, start, orientation):  # placement of word in xword board
    ...
    brd, seen = ''.join(brd), set()
    for key in POSSIBLE_POSITIONS:  # go through every word on the board to check for any invalid words
        positions = POSSIBLE_POSITIONS[key]
        word = brd[positions[0]: positions[-1] + 1: key[1]]
        if word in ALL_WORDS and word not in seen: seen.add(word) # if word exists and is unique in this xword, it is valid
...
def update_word_choices(brd, positions, orientation, possible_positions, possible_words, used_word):
    ...
    for pos in positions:
        ...
        updated_words[(start, opposite)] = [w for w in possible_words[(start, opposite)] if w[(pos - start) // opposite] == brd[pos] and w != used_word]
