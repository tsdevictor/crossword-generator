...
def parse_args():
    ...
    # PREFIXES = {2: {c1 + c2 for c1 in alphabet for c2 in alphabet if c1 + c2 in dict_string}}
    # PREFIXES[3] = {pre + c for pre in PREFIXES[2] for c in alphabet if pre + c in dict_string}
...
def set_up(brd):  # pre-compute possible words at each position
    global POSSIBLE_WORDS, POSSIBLE_POSITIONS
    ...
    for i, char in enumerate(brd):
        ...
        if i // W == 0 or (i - W >= 0 and brd[i - W] == '#'):
            ...
            orientations.append(((hole.find('#') + i // W) * W + i % W if '#' in hole else BRD_SIZE - (W - i % W) + 1, W))
        if i % W == 0 or ((i - 1) // W == i // W and brd[i - 1] == '#'):
            orientations.append((min(brd.find('#', i) if NUM_BLOCKING and '#' in brd[i:] else BRD_SIZE, i // W * W + W), 1))
        for end, orientation in orientations:
            if '-' not in (spec := brd[i: end: orientation]): continue
            POSSIBLE_POSITIONS[(i, orientation)] = range(i, end, orientation)
            POSSIBLE_WORDS[(i, orientation)] = []
            for w in WORDS_BY_LEN[len(spec)]:
                for k, c in enumerate(w):
                    if spec[k] not in {'-', c}: break
                else: POSSIBLE_WORDS[(i, orientation)].append(w)
...
def get_word(pos, orientation, possible_words, spec, used_words):  # return word that can fill given position
    ...
    for word in possible_words[(pos, orientation)]:
...
def is_valid(brd):  # check that each word (i) exists or (ii) is not yet complete
    for start, orientation in POSSIBLE_WORDS:
        word = ''.join(brd[i] for i in POSSIBLE_POSITIONS[(start, orientation)])
        if '-' in word:
            # if 2 <= word.find('-') <= 4:
            #     if word[:word.find('-')] not in PREFIXES[word.find('-')]: return False
            continue
        if word not in ALL_WORDS: return False
    return True


def fill_words(brd, possible_positions, possible_words, filled_positions, used_words):  # brute force filling of words in xword
    ...
    for start, orientation in sorted(possible_positions.keys(), key=lambda k: len(possible_words[k])):
        if (start, orientation) in filled_positions: continue
        ...
        new_brd = fill_words(new_brd, {k: possible_positions[k] for k in possible_positions if k != (start, orientation)}, possible_words, filled_positions | {(start, orientation)}, used_words | {word})
        if new_brd: return new_brd
...
def main():
    ...
    two_d_print(brd)
    set_up(brd)
    brd = fill_words(brd, POSSIBLE_POSITIONS, POSSIBLE_WORDS, set(), set())
    print(brd)
