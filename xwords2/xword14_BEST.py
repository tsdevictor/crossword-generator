...
def update_words(brd, positions, orientation, possible_positions, possible_words, used_words):
    ...
    updated_words = possible_words.copy()
    updated_positions = possible_positions.copy()
    updated_positions.pop((positions[0], orientation))
    for pos in positions:
        if (pos, opposite) not in POS_TO_WORD_START: continue
        start1 = POS_TO_WORD_START[(pos, opposite)]
        # update based on whether word satisfies prefix above it
        possible = []
        for word in possible_words[(start1, opposite)]:
            if word[pos // opposite - start1 // opposite] == brd[pos] and word not in used_words:
                possible.append(word)
        updated_words[(start1, opposite)] = possible
