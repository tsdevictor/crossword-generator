...
def set_up(brd):  # pre-compute possible words at each position
    global POSSIBLE_WORDS, POSSIBLE_POSITIONS, MAX_SCORE, POS_TO_WORD_START
    MAX_SCORE = 0
...
def is_valid(brd):  # check that each word (i) exists or (ii) is not yet complete
    ...
    for start, orientation in POSSIBLE_WORDS:
        word = ''
        for i in POSSIBLE_POSITIONS[(start, orientation)]:
            if brd[i] == '-': break
            word += brd[i]
        else:
            if word not in ALL_WORDS: return False, score
            score += len(word)
    return True, score
...
def update_words(brd, positions, orientation, possible_words, used_words):
    ...
    for pos in positions:
        ...
        updated[(start, opposite)] = [w for w in possible_words[(start, opposite)] if
                                      w[pos // opposite - start // opposite] == brd[pos] or w in used_words]
    return updated
    ...
    for start, orientation in sorted(possible_positions.keys(), key=lambda k: len(possible_words[k])):
        ...
        for word in possible_words[(start, orientation)]:
            ...
            for i in range(len(word)):
                if spec[i] != word[i] and spec[i] != '-': break
            else:
