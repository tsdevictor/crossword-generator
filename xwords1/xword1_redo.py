...
import re
import cProfile
args = ['15x15', '39', 'H0x0Mute', 'V0x0mule', 'V10x13Evite', 'H7x5#', 'V3x4#', 'H6x7#', 'V11x3#']
...
def solve(brd):
    ...
    for i, char in enumerate(brd):
        ...
        word = get_word(''.join(brd[i: i + word_len * W: W]), used_words)
        used_words.add(word)
        for c in word:
            brd, i = brd[:i] + c + brd[i + 1:], i + W
        break
    ...
    for i, char in enumerate(brd):
        ...
        word = get_word(brd[i: i + word_len], used_words)
        used_words.add(word)
        for c in word:
            brd, i = brd[:i] + c + brd[i + 1:], i + 1
