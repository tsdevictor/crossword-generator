...
def connected(brd, seen):
    seen = floodfill(brd, brd.find('-'), seen)[1]
    return len(seen) == H*W-brd.count('#')


def floodfill(brd, pos, seen):
    if not 0 <= pos < H * W: return True, []
    if pos in seen: return True, []
    if brd[pos] == '#': return True, []

    seen.add(pos)
    return floodfill(brd, pos + W, seen) \
        and floodfill(brd, pos - W, seen) \
        and floodfill(brd, pos + 1, seen) \
        and floodfill(brd, pos - 1, seen), seen
