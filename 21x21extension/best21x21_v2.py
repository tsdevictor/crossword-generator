...
args = ['dctEckel.txt', '14x16', '108', 'H3x2', 'h2x4', 'v4x5', 'v7x5', 'V9x7', 'V9x5']
...
def parse_args():
    ...
    # pre-computation of neighbor positions (1 square north, east, south, west)
    FLOODFILL_NBRS = [[] for _ in range(BRD_SIZE)]
    for i in range(BRD_SIZE):
        nbrs = []
        if (i + 1) // W == i // W: nbrs.append(i + 1)
        if (i - 1) // W == i // W: nbrs.append(i - 1)
        nbrs.append(i + W)
        nbrs.append(i - W)
        FLOODFILL_NBRS[i] = [n for n in nbrs if 0 <= n < BRD_SIZE]
